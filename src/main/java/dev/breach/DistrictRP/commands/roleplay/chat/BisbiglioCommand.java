...
public class BisbiglioCommand implements CommandExecutor {

    private final DistrictRP plugin;

    public BisbiglioCommand(DistrictRP plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        double radius = plugin.getConfig().getDouble("chat.formats.bisbiglio.radius", 15);

        boolean anonymous = ChatFlags.isAnonymous(player);
        String formatKey = anonymous
                ? "chat.formats.bisbiglio.anonymous"
                : "chat.formats.bisbiglio.format";
