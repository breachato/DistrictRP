...
public class MondoCommand implements CommandExecutor {
    ...
    public MondoCommand(DistrictRP plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        if (!sender.hasPermission("DistrictRP.mondo")) {
    ...
    private void handleVisit(CommandSender s, String[] args) {
        ...
        if (plugin.getDataManager().isBlacklisted(name, uuid)
                && !p.hasPermission("DistrictRP.mondo.bypass")) {
    ...
    private void handleList(CommandSender s) {
        ...
        MessageUtils.send(s, "");
        MessageUtils.send(s, "&e&l  ᴍᴏɴᴅɪ ʀᴇɢɪꜱᴛʀᴀᴛɪ");
        MessageUtils.send(s, "");
        if (worlds.isEmpty()) {
            ...
            for (String w : worlds) {
                ...
                MessageUtils.send(s, "&e  ▸ &f" + w
                        + " &8(WL: &e" + wl + " &8| BL: &e" + bl + "&8)");
            }
        }
        MessageUtils.send(s, "");
    }
    ...
    private void handleWhitelist(CommandSender s, String[] args) {
        ...
        if (action.equals("aggiungi") || action.equals("add")) {
            ...
            MessageUtils.send(s, "");
            MessageUtils.send(s, "&e&l  ✔ WHITELIST &f" + world);
            MessageUtils.send(s, "");
            if (list.isEmpty()) {
    ...
    private void handleBlacklist(CommandSender s, String[] args) {
        ...
        if (action.equals("aggiungi") || action.equals("add")) {
            ...
            MessageUtils.send(s, "");
            MessageUtils.send(s, "&e&l  ✖ BLACKLIST &f" + world);
            MessageUtils.send(s, "");
            if (list.isEmpty()) {
    ...
    private void sendHelp(CommandSender s) {
        MessageUtils.send(s, "");
        MessageUtils.send(s, "&e&l   DistrictRP");
        MessageUtils.send(s, "");
        MessageUtils.send(s, "&e  /mondo aggiungi <cartella|url> [nome]");
        ...
        MessageUtils.send(s, "");
        MessageUtils.send(s, "&e  /mondo rimuovi <nome>");
        MessageUtils.send(s, "&e  /mondo visita <nome>");
        MessageUtils.send(s, "&e  /mondo lista");
        MessageUtils.send(s, "");
        MessageUtils.send(s, "&e  /mondo whitelist <aggiungi|rimuovi|lista> <mondo> [player]");
        MessageUtils.send(s, "&e  /mondo blacklist <aggiungi|rimuovi|lista> <mondo> [player]");
        MessageUtils.send(s, "");
    }
}
