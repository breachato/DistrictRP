...
public class GodCommand implements CommandExecutor {
    ...
    public GodCommand(DistrictRP plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        if (!sender.hasPermission("DistrictRP.god")) {
        ...
        boolean other = false;
        if (args.length > 0) {
            if (!sender.hasPermission("DistrictRP.god.others")) {
        ...
        if (newState) {
            ...
            if (other) MessageUtils.sendPrefixed(sender, "&fGodmode attivato per &e" + target.getName() + "&f.");
        } else {
            plugin.godMode.remove(uuid);
            MessageUtils.sendPrefixed(target, "&cGodmode disattivato.");
            if (other) MessageUtils.sendPrefixed(sender, "&cGodmode disattivato per &e" + target.getName() + "&c.");
        }
        return true;
    }
}